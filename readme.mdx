...
If you think of each line of code or change as an item in an order, then a commit is a neat little packaged cardboard box containing those items (changes), with a "package label" referred to as a `commit message` and a random unique ID known as `commit number` (despite it also having letters) about the contents of the box.
